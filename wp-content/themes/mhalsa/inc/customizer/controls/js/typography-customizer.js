( function( api ) {
	...
	api.controlConstructor['mhalsa-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.mhalsa-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the mhalsa_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'mhalsa_get_all_google_fonts_ajax',
								mhalsa_customize_nonce: mhalsa_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.mhalsa-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.mhalsa-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.mhalsa-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.mhalsa-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.mhalsa-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.mhalsa-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.mhalsa-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.mhalsa-font-variant' );
			...
			control.container.on( 'change', '.mhalsa-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.mhalsa-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.mhalsa-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.mhalsa-font-family select' ).selectWoo();
	jQuery( '.mhalsa-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".mhalsa-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.mhalsa-font-variant' ).hide();
		}
	});

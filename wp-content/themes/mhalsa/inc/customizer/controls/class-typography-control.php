...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Mhalsa_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Mhalsa_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'mhalsa-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'mhalsa-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), MHALSA_VERSION, true );
			wp_enqueue_style( 'mhalsa-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), MHALSA_VERSION );

			wp_enqueue_script( 'mhalsa-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'mhalsa-typography-selectWoo' ), MHALSA_VERSION, true );
			wp_enqueue_style( 'mhalsa-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), MHALSA_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'mhalsa_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'mhalsa' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'mhalsa' );
			$this->json[ 'google_fonts' ] = apply_filters( 'mhalsa_typography_customize_list', mhalsa_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = mhalsa_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'mhalsa' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'mhalsa' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'mhalsa' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'mhalsa' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="mhalsa-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="mhalsa-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="mhalsa-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="mhalsa-font-transform">
					<label>

...
add_action( 'after_setup_theme', 'mhalsa_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function mhalsa_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'mhalsa_construct_sidebars' );
}

if ( ! function_exists( 'mhalsa_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'mhalsa_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function mhalsa_woocommerce_start() { ?>
...
if ( ! function_exists( 'mhalsa_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'mhalsa_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function mhalsa_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'mhalsa_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'mhalsa_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function mhalsa_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'mhalsa_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'mhalsa_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'mhalsa_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function mhalsa_bbpress_css() {
...
if ( ! function_exists( 'mhalsa_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'mhalsa_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function mhalsa_buddypress_css() {
...
if ( ! function_exists( 'mhalsa_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'mhalsa_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function mhalsa_beaver_builder_css() {
